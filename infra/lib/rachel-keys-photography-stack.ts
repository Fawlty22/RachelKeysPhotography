...
export class RachelKeysPhotographyStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Automated rebuild pipeline — triggered by the CMS on publish.
    // No GitHub connection needed; CodeBuild pulls source from S3.
    // After deploying, upload the initial source bundle (see SourceBucketName output):
    ...
    const pipeline = new PortfolioPipeline(this, "PortfolioPipeline", {
    ...
    new route53.ARecord(this, "PortfolioARecord", {
      ...
      target: route53.RecordTarget.fromAlias(
        new targets.CloudFrontTarget(portfolio.distribution),
      ),
    });
    ...
    new route53.ARecord(this, "CmsARecord", {
      ...
      recordName: "admin",
      target: route53.RecordTarget.fromAlias(
        new targets.CloudFrontTarget(cms.distribution),
      ),
    });
    ...
    new route53.ARecord(this, "PhotosARecord", {
      ...
      recordName: "photos",
      target: route53.RecordTarget.fromAlias(
        new targets.CloudFrontTarget(photosCdn.distribution),
      ),
    });
    ...
    new cdk.CfnOutput(this, "PortfolioBucketName", {
      value: portfolio.bucket.bucketName,
    });
    new cdk.CfnOutput(this, "CmsBucketName", { value: cms.bucket.bucketName });
    new cdk.CfnOutput(this, "PhotosBucketName", {
      value: photos.bucket.bucketName,
    });
    new cdk.CfnOutput(this, "PortfolioDistributionUrl", {
      value: `https://${portfolio.distribution.distributionDomainName}`,
    });
    new cdk.CfnOutput(this, "CmsDistributionUrl", {
      value: `https://${cms.distribution.distributionDomainName}`,
    });
    new cdk.CfnOutput(this, "PhotosCdnUrl", {
      value: "https://photos.rachelkeysphotography.com",
    });
    new cdk.CfnOutput(this, "HostedZoneId", {
      value: dns.hostedZone.hostedZoneId,
    });
    ...
    new cdk.CfnOutput(this, "PortfolioRebuildTriggerUrl", {
      value: pipeline.triggerUrl,
      description:
        "POST to this URL (with X-Rebuild-Secret header) to trigger a portfolio rebuild",
    });
    new cdk.CfnOutput(this, "PortfolioSourceBucketName", {
